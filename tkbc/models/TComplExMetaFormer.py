...
class FourierTransform(nn.Module):
    """
    Fourier Transform token mixer with integrated MLPs for complex numbers and spatial domain
    Performs FFT -> frequency domain processing -> MLP (complex numbers) -> inverse FFT -> MLP (spatial domain)
    """
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, drop=0., **kwargs):
        ...
        # MLP for spatial domain (integrated from Mlp class)
        self.fc1_spatial = nn.Linear(dim, mlp_hidden_dim)
        self.fc2_spatial = nn.Linear(mlp_hidden_dim, dim)
        self.drop_spatial = nn.Dropout(drop)

    def forward(self, x):
        """
        Apply Fourier transform token mixing with MLPs for complex numbers and spatial domain
        ...
        # Apply spatial domain MLP
        if len(original_shape) == 4:
            # Reshape for spatial MLP processing
            x_reconstructed = x_reconstructed.permute(0, 2, 1).reshape(B * H * W, C)
            x_reconstructed = self.fc1_spatial(x_reconstructed)
            x_reconstructed = self.act(x_reconstructed)
            x_reconstructed = self.drop_spatial(x_reconstructed)
            x_reconstructed = self.fc2_spatial(x_reconstructed)
            x_reconstructed = self.drop_spatial(x_reconstructed)
            x_reconstructed = x_reconstructed.reshape(B, H, W, C).permute(0, 3, 1, 2)
        else:
            x_reconstructed = self.fc1_spatial(x_reconstructed)
            x_reconstructed = self.act(x_reconstructed)
            x_reconstructed = self.drop_spatial(x_reconstructed)
...
class MetaFormerBlock(nn.Module):
    """
    MetaFormer block with Fourier Transform token mixer (including spatial MLP)
    """
    def __init__(self, dim, 
        ...
        self.token_mixer = token_mixer(dim=dim, mlp_ratio=mlp_ratio, act_layer=act_layer, drop=drop)
...
class TComplExMetaFormer(TKBCModel):
    """
    TComplEx model enhanced with MetaFormer architecture using Fourier Transform token mixing
    """
    def __init__(
        self,
        ...
        super(TComplExMetaFormer, self).__init__()
        self.sizes = sizes
        self.rank = rank
    ...
    def score(self, x):
        ...
        # Split into real and imaginary parts (original TComplEx logic)
    ...
    def forward(self, x):
        ...
        # Compute relation-time interaction (original TComplEx style)
    ...
    def forward_over_time(self, x):
        ...
        # TComplEx forward over time computation
        return (
